...
public class MainActivity extends AppCompatActivity {
    ...
    private Button sendButtonwl;
    private Button sendButtonwyl;
...
//        messageEditText = (EditText) findViewById(R.id.message_edit_text);
        sendButtonwl = (Button) findViewById(R.id.send_buttonwl);
        sendButtonwyl = (Button) findViewById(R.id.send_buttonwyl);
...
//        pickContact = (ImageView) findViewById(R.id.add_contact_image_view);
        sendButtonwl.setOnClickListener(new View.OnClickListener() {
            @Override
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    ...
                    if (hasSMSPermission != PackageManager.PERMISSION_GRANTED) {
                        if (!shouldShowRequestPermissionRationale(Manifest.permission.SEND_SMS)) {
                            showMessageOKCancel("Najpierw musisz zaakceptować wysyłanie smsów!",
        ...
        sendButtonwyl.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
...
//        });
    ...
    public void onResume() {
        ...
        deliveredStatusReceiver=new BroadcastReceiver() {
            ...
            @Override
            public void onReceive(Context arg0, Intent arg1) {
                String s = "Wiadomość nie wysłana";
                switch(getResultCode()) {
                    case Activity.RESULT_OK:
                        s = "Wiadomość wysłana";
                        break;
        ...
        switch (requestCode) {
            case REQUEST_SMS:
                if (grantResults.length > 0 &&  grantResults[0] == PackageManager.PERMISSION_GRANTED){
                        Toast.makeText(getApplicationContext(), "Teraz możesz wysłać smsa", Toast.LENGTH_SHORT).show();
                }else {
                    Toast.makeText(getApplicationContext(), "Odmowa dostępu, musisz odblokować smsy", Toast.LENGTH_SHORT).show();
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        if (shouldShowRequestPermissionRationale(SEND_SMS)) {
                            showMessageOKCancel("Musisz zaakceptować oba warunki",
                                    new DialogInterface.OnClickListener() {
                                        @Override
                                        public void onClick(DialogInterface dialog, int which) {
                                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
